...
@HiltWorker
class TransactionExtractionWorker @AssistedInject constructor(
    ...
    companion object {
        private const val TAG = "TransactionExtractionWorker"
    }

    init {
        Log.e(TAG, "WORKER CREATED - TransactionExtractionWorker instantiated")
    }

    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        try {
            Log.e(TAG, "WORKER STARTED - doWork() called")
            ...
            val transaction = smsTransactionParser.parseTransactionFromSms(smsBody, smsAddress, smsTimestamp)
            
            if (transaction != null) {
                Log.d(TAG, "Transaction parsed successfully: ${transaction.description}")
